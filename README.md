...
- **Fondo de detracciones:** `/resumen-mensual` lleva el saldo de la cuenta del Banco de la Nación. **Abonan solo las detracciones de las ventas** —las de compras van a la cuenta del proveedor—, y carga lo que se usa para pagar tributos, que se anota al registrar el pago del mes (columna *Pagado c/ detracc.*, con un botón «máx» que gasta del fondo hasta cubrir el pago). El saldo no se guarda en ninguna columna: se calcula sumando abonos y usos, así que no puede quedar desfasado, y se arrastra entre años.
...
- **Redondeo del 0621:** SUNAT **liquida cada tributo sobre la base ya redondeada** (con base 2 330,51 e IGV 419,49 declara 2 331 y 420, no 419) y encadena el saldo a favor con esos enteros. Hace lo mismo con el crédito fiscal —una compra de 8,47 con IGV 1,53 la propone como 107 = 8 y 108 = 1, porque 8 × 18% = 1,44— y con el pago a cuenta de renta (301 × 315: 691 × 1% = 6,91 → 7). Operar con céntimos y redondear al final se desvía hasta un sol, así que el resumen mensual muestra las cifras de SUNAT cuando el interruptor está en *Sin decimales (SUNAT)* y las exactas cuando está en *Con decimales*. Si el crédito real de tus compras es menor que el que propone el formulario, la guía lo avisa y la casilla se corrige a mano.
- **Qué pagar cada mes:** en `/resumen-mensual` las columnas siguen el orden del formulario: *IGV del mes* (casilla 140, débito menos crédito), *Saldo Ant.* (145), *Resultado IGV* (184: el impuesto o el saldo a favor que se arrastra) y *Pagar IGV* / *Pagar IR* (189 / 307), lo que sale del bolsillo (IGV en 0 cuando el saldo a favor lo cubre); junto a *Pagado IGV* / *Pagado IR* se compara con lo registrado. Si SUNAT no precarga la casilla 145, hay que escribir el saldo a favor del mes anterior: con 0, el formulario cobra IGV que ese saldo ya cubría. La guía enseña también cuánto pediría el formulario con la 145 en 0. Las columnas ocultas y el modo de decimales se recuerdan entre recargas.
- **Guía de declaración 0621:** `/resumen-mensual` indica casilla por casilla qué poner (100/101 y 154/155 en ventas, 107/108 y 156/157 en compras, 140, 145, 184 y 189 en la determinación, y 301, 315, 302 y 307 en renta) y avisa **antes** de abrir el formulario si el tributo declarado quedaría fuera de la banda que valida SUNAT.
