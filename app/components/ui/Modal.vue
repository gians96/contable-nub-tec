<template>
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <!-- Modal -->
      <div class="relative bg-surface rounded-xl shadow-xl w-full z-10 max-h-[90vh] flex flex-col"
        :class="sizeClass">
        <!-- Header -->
        <div class="flex items-center justify-between px-6 py-4 border-b border-line shrink-0">
          ...
          <button @click="close" class="p-1 rounded-lg hover:bg-surface-muted">
        ...
        <!-- Footer -->
        <div v-if="$slots.footer" class="px-6 py-4 border-t border-line shrink-0">
          <slot name="footer" />
        </div>
      </div>
    </div>
